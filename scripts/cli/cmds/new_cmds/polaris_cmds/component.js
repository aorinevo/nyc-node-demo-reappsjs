var props = require('../../../../../reapps-properties.json'),
    component = require('../../../../polaris/component.js'),
    winston = require('winston');
    
exports.command = 'component [n] [s] [t] [v]'
exports.desc = 'Creates view [v], scss [s], spec [t], and hbs, with filename based on [n]. To skip a file type add the corresponding flag.'
exports.builder = {
  'name': {
    demandOption: true,
    alias: 'n'
  },
  'view': {
    default: false,
    alias: 'v'
  },
  'stylesheet': {
    default: false,
    alias: 's'
  },
  'spec': {
    default: false,
    alias: 't'
  },
  'templates': {
    default: false,
    alias: 'b'
  }
}
exports.handler = function (argv) {
  if(!argv.v){
    component.new.view( props.paths["creditGateway"], argv.n);
  }
  if(!argv.v){
    component.new.scss( props.paths["creditGateway"], argv.n);
  }
  if(!argv.b){
    component.new.hbs( props.paths["creditGateway"], argv.n);
  }
}