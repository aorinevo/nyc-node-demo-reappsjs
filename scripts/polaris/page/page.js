...
function deleteHelper( pathToProj, name, directory ){
  var  projName = pathToProj.split("/").slice(-1)[0];
  ['common','bcom','mcom'].forEach(function(item){
    shell.exec(`rm -rf ${pathToProj}/${directory}/${projName}/${item}/pages/${name}`);
  });
}

function newView( pathToProj, name ){
  newHelper(pathToProj, '../../../templates/polaris/page/view.js', name, 'src', `${name}View.js`).then(function(results){
...
function newScss( pathToProj, name ){
  newHelper(pathToProj, '../../../templates/polaris/page/scss.js', name, 'scss', `${name}.scss`).then(function(results){
...
function newHbs( pathToProj, name ){
  newHelper(pathToProj, '../../../templates/polaris/page/hbs.js', name, 'views/templates', `${name}.hbs`).then(function(results){
...
function newSpec( pathToProj, name ){
  newHelper(pathToProj, '../../../templates/polaris/page/spec.js', name, 'tests', `${name}.spec.js`).then(function(results){
