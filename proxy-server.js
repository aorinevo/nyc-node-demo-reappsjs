...
function updateServerBlocks( proxyServer, fileName, extension ){
  ...
  if( !fs.existsSync(`${proxyServer.path}/servers/${fileName}.${extension}`) || argv.force ){
    fs.writeFile( `./${fileName}.${extension}`, serverBlocks[proxyServer.name][fileName], 'utf8', function (err) {
        if (err) {
         winston.log('error', err);
         return false;
        }
        shell.exec(`sudo mv ./${fileName}.${extension} ${proxyServer.path}/servers/${fileName}.${extension}` );
       winston.log( 'info', `${fileName}.${extension} file created in ${proxyServer.path}/servers/` );
       if( proxyServer.name === 'apache24' ){
        shell.exec('sudo apachectl restart');
       } else {
        shell.exec('sudo nginx -s stop');
    ...
    winston.log( 'info', `/etc/apache2/servers/${fileName}.${extension} already exists.`);
    winston.log( 'info', `To overwrite file(s), use --force`);
  }
}
